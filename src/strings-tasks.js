...
function isPalindrome(str) {
  const mystr = str
    ...
    .split(' ')
    .join('');
  const reversed = mystr.split('').reverse().join('');
  ...
  str.split(' ').forEach((word) => {
    ...
    word.split('').forEach((char) => {
      if (alphabet.includes(char)) {
        reversedWord.push(reversedAlphabet.charAt(alphabet.indexOf(char)));
      } else {
        reversedWord.push(char);
      }
    });
    reversedRow.push(reversedWord.join(''));
  });

  return reversedRow.join(' ');
}
